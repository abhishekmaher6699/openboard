...
export default function PixiBoard({ boardId }: { boardId: string }) {
  ...
  const { sendMove, sendManyMoves, sendDelete, sendManyDelete, sendCreate, sendResize, sendResizeMany, sendColorUpdate, sendTextUpdate } =
  ...
  const moveManyObjects = async (moves: { id: string; x: number; y: number }[]) => {
    setObjects((prev) => prev.map((obj) => { const move = moves.find((m) => m.id === obj.id); return move ? { ...obj, x: move.x, y: move.y } : obj; }));
    try {
      sendManyMoves(moves);
  ...
  const deleteObject = async (id: string) => {
    ...
    try { await deleteObjectApi(boardId, id); sendDelete(id); }
    catch (err) { console.error("Delete failed", err); }
  };

  const deleteManyObjects = async (ids: string[]) => {
    setObjects((prev) => prev.filter((obj) => !ids.includes(obj.id)));
    try { sendManyDelete(ids); await Promise.all(ids.map((id) => deleteObjectApi(boardId, id))); }
  ...
  const resizeObject = async (id: string, width: number, height: number, x: number, y: number) => {
    ...
    try { sendResize(id, width, height, x, y); await updateObject(boardId, id, { width, height, x, y }); }
